...
Explain 是一個 evidence-first 的理解介面。它把技術系統、spec、diff、workflow、
artifact 或多輪進度，整理成一份可以一次讀懂的 explanation packet：先說這是什麼、
為什麼重要，再畫出結構與機制，最後把邊界、未完成項目與容易誤解的地方講清楚。

它的工作不是把內容變得「比較淺」，而是把複雜度重新組織成可理解的形狀。
...
- 找 bug 或 correctness 問題
- code review、audit 或 blame analysis
- challenge 一份計畫
- 直接實作或修改程式
- 產生測驗或考使用者

Explain 是 comprehension interface，不是 correctness review。使用者同時要求 review
時，應該使用適合的 review workflow，再用 Explain 呈現結果。
